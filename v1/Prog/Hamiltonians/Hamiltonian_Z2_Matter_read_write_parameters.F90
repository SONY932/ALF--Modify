...
! Supplies subroutine for automatically reading and writing the parameters of "Z2_Matter"
      ...
      Subroutine read_parameters()
         ...
         NAMELIST /VAR_Z2_Matter/  N_SUN, ham_T, ham_TZ2, Ham_chem, Ham_U, &
              &     Ham_J, Ham_K, Ham_h, Ham_g, Dtau, Beta, Projector, &
              &     Theta, N_part, UseStrictGauss, GaussSector
         ...
         Model        = "Z2_Matter" ! Possible Values: 'Z2_Matter'
         Lattice_type = "Square"    ! Possible Values: 'Square'
         L1           = 6           ! Length in direction a_1
         L2           = 6           ! Length in direction a_2
         !Parameters VAR_Z2_Matter
         N_SUN          = 2       ! 
         ham_T          = 1.0d0   ! Hopping for fermions
         ham_TZ2        = 1.0d0   ! Hopping for orthogonal fermions
         Ham_chem       = 0.0d0   ! Chemical potential for fermions
         Ham_U          = 0.0d0   ! Hubbard for fermions
         Ham_J          = 1.0d0   ! Hopping Z2 matter fields
         Ham_K          = 1.0d0   ! Plaquette term for gauge fields
         Ham_h          = 1.0d0   ! sigma^x-term for matter
         Ham_g          = 1.0d0   ! tau^x-term for gauge
         Dtau           = 0.1d0   ! Thereby Ltrot=Beta/dtau
         Beta           = 10.0d0  ! Inverse temperature
         Projector      = .false. ! Whether the projective algorithm is 
                                  ! used
         Theta          = 0.0d0   ! Projection parameter
         N_part         = -1      ! Number of particles in trial wave function. 
                                  ! If N_part < 0 -> N_part = L1*L2/2
         UseStrictGauss = .false. ! Whether to use strict Gauss constraint 
                                  ! projection
         GaussSector    = "even"  ! Gauss sector: 'even' (Q_r=+1), 'odd' 
                                  ! (Q_r=-1), 'staggered'
#ifdef MPI
         If (Irank_g == 0 ) then
#endif
            ...
            REWIND(unit_para)
            READ(unit_para, NML=VAR_Z2_Matter)
            close(unit_para)
         ...
         CALL MPI_BCAST(UseStrictGauss,  1,MPI_LOGICAL  ,0,Group_Comm,ierr)
         CALL MPI_BCAST(GaussSector   , 64,MPI_CHARACTER,0,Group_Comm,ierr)
#endif
...
CALL h5lexists_f(group_id, "var_z2_matter", link_exists, ierr)
                       if ( .not. link_exists ) then
                         call h5gcreate_f(group_id, "var_z2_matter", group_id2, ierr)
                         call h5gclose_f(group_id2, ierr)
                       endif
         call test_attribute(group_id, "var_z2_matter", "n_sun", N_SUN, ierr)
         call test_attribute(group_id, "var_z2_matter", "ham_t", ham_T, ierr)
         call test_attribute(group_id, "var_z2_matter", "ham_tz2", ham_TZ2, ierr)
         call test_attribute(group_id, "var_z2_matter", "ham_chem", Ham_chem, ierr)
         call test_attribute(group_id, "var_z2_matter", "ham_u", Ham_U, ierr)
         call test_attribute(group_id, "var_z2_matter", "ham_j", Ham_J, ierr)
         call test_attribute(group_id, "var_z2_matter", "ham_k", Ham_K, ierr)
         call test_attribute(group_id, "var_z2_matter", "ham_h", Ham_h, ierr)
         call test_attribute(group_id, "var_z2_matter", "ham_g", Ham_g, ierr)
         call test_attribute(group_id, "var_z2_matter", "dtau", Dtau, ierr)
         call test_attribute(group_id, "var_z2_matter", "beta", Beta, ierr)
         call test_attribute(group_id, "var_z2_matter", "projector", Projector, ierr)
         call test_attribute(group_id, "var_z2_matter", "theta", Theta, ierr)
         call test_attribute(group_id, "var_z2_matter", "n_part", N_part, ierr)
         call test_attribute(group_id, "var_z2_matter", "usestrictgauss", UseStrictGauss, ierr)
         call test_attribute(group_id, "var_z2_matter", "gausssector", GaussSector, ierr)
