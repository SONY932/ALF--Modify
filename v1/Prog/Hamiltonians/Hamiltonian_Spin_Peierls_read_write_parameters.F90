...
! Supplies subroutine for automatically reading and writing the parameters of "Spin_Peierls"
      ...
      Subroutine read_parameters()
         ...
         NAMELIST /VAR_Model_Generic/  N_SUN, N_FL, Phi_X, Phi_Y, Bulk, &
              ...
              &     Theta
         NAMELIST /VAR_Spin_Peierls/  Ham_Jx, Ham_Jy, Ham_U, Ham_h, Ham_g_factor, &
              &     Ham_Lambda, Ham_Omega0
         ...
         Model        = "Spin_Peierls" ! Value not relevant
         Lattice_type = "Square"       ! 
         ...
         If (Irank_g == 0 ) then
            ...
            REWIND(unit_para)
            READ(unit_para, NML=VAR_Spin_Peierls)
            close(unit_para)
         ...
         CALL MPI_BCAST(Ham_Jx      ,  1,MPI_REAL8    ,0,Group_Comm,ierr)
         CALL MPI_BCAST(Ham_Jy      ,  1,MPI_REAL8    ,0,Group_Comm,ierr)
         CALL MPI_BCAST(Ham_U       ,  1,MPI_REAL8    ,0,Group_Comm,ierr)
         CALL MPI_BCAST(Ham_h       ,  1,MPI_REAL8    ,0,Group_Comm,ierr)
         CALL MPI_BCAST(Ham_g_factor,  1,MPI_REAL8    ,0,Group_Comm,ierr)
         CALL MPI_BCAST(Ham_Lambda  ,  1,MPI_REAL8    ,0,Group_Comm,ierr)
         CALL MPI_BCAST(Ham_Omega0  ,  1,MPI_REAL8    ,0,Group_Comm,ierr)
#endif
...
CALL h5lexists_f(group_id, "var_spin_peierls", link_exists, ierr)
                       if ( .not. link_exists ) then
                         call h5gcreate_f(group_id, "var_spin_peierls", group_id2, ierr)
                         call h5gclose_f(group_id2, ierr)
                       endif
         call test_attribute(group_id, "var_spin_peierls", "ham_jx", Ham_Jx, ierr)
         call test_attribute(group_id, "var_spin_peierls", "ham_jy", Ham_Jy, ierr)
         call test_attribute(group_id, "var_spin_peierls", "ham_u", Ham_U, ierr)
         call test_attribute(group_id, "var_spin_peierls", "ham_h", Ham_h, ierr)
         call test_attribute(group_id, "var_spin_peierls", "ham_g_factor", Ham_g_factor, ierr)
         call test_attribute(group_id, "var_spin_peierls", "ham_lambda", Ham_Lambda, ierr)
         call test_attribute(group_id, "var_spin_peierls", "ham_omega0", Ham_Omega0, ierr)
