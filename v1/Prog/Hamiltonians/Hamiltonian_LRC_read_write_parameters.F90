...
! Supplies subroutine for automatically reading and writing the parameters of "LRC"
      ...
      Subroutine read_parameters()
         ...
         NAMELIST /VAR_Model_Generic/  N_SUN, N_FL, Phi_X, Phi_Y, Bulk, &
              ...
              &     Theta
         NAMELIST /VAR_LRC/  ham_T, ham_T2, ham_Tperp, Ham_chem, Ham_U, &
              &     ham_alpha, Percent_change
         ...
         Ham_chem       = 1.0d0 ! Chemical potential
         Ham_U          = 4.0d0 ! On-site interaction
         ham_alpha      = 0.1d0 ! Coulomb tail magnitude
         Percent_change = 0.1d0 ! Parameter P
#ifdef MPI
         If (Irank_g == 0 ) then
#endif
#ifdef TEMPERING
...
CALL h5lexists_f(group_id, "var_lrc", link_exists, ierr)
                       if ( .not. link_exists ) then
                         call h5gcreate_f(group_id, "var_lrc", group_id2, ierr)
                         call h5gclose_f(group_id2, ierr)
                       endif
         call test_attribute(group_id, "var_lrc", "ham_t", ham_T, ierr)
         call test_attribute(group_id, "var_lrc", "ham_t2", ham_T2, ierr)
         call test_attribute(group_id, "var_lrc", "ham_tperp", ham_Tperp, ierr)
         call test_attribute(group_id, "var_lrc", "ham_chem", Ham_chem, ierr)
         call test_attribute(group_id, "var_lrc", "ham_u", Ham_U, ierr)
         call test_attribute(group_id, "var_lrc", "ham_alpha", ham_alpha, ierr)
         call test_attribute(group_id, "var_lrc", "percent_change", Percent_change, ierr)
