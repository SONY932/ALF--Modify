...
! Supplies subroutine for automatically reading and writing the parameters of "Kondo"
      ...
      Subroutine read_parameters()
         ...
         Lattice_type = "Bilayer_square" ! Possible values: 'Bilayer_square', 
                                         ! 'Bilayer_honeycomb'
         ...
         !Parameters VAR_Kondo
         ham_T    = 1.0d0 ! Hopping parameter
         Ham_chem = 0.0d0 ! Chemical potential
         Ham_Uc   = 0.0d0 ! Hubbard interaction  on  c-orbitals Uc
         ham_Uf   = 2.0d0 ! Hubbard interaction  on  f-orbials  Uf
         Ham_JK   = 2.0d0 ! Kondo Coupling  J
#ifdef MPI
         If (Irank_g == 0 ) then
#endif
#ifdef TEMPERING
...
CALL h5lexists_f(group_id, "var_kondo", link_exists, ierr)
                       if ( .not. link_exists ) then
                         call h5gcreate_f(group_id, "var_kondo", group_id2, ierr)
                         call h5gclose_f(group_id2, ierr)
                       endif
         call test_attribute(group_id, "var_kondo", "ham_t", ham_T, ierr)
         call test_attribute(group_id, "var_kondo", "ham_chem", Ham_chem, ierr)
         call test_attribute(group_id, "var_kondo", "ham_uc", Ham_Uc, ierr)
         call test_attribute(group_id, "var_kondo", "ham_uf", ham_Uf, ierr)
         call test_attribute(group_id, "var_kondo", "ham_jk", Ham_JK, ierr)
