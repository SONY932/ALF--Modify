...
! Supplies subroutine for automatically reading and writing the parameters of "Hubbard"
      ...
      Subroutine read_parameters()
         ...
         NAMELIST /VAR_Model_Generic/  N_SUN, N_FL, Phi_X, Phi_Y, Bulk, &
              ...
              &     Theta
         NAMELIST /VAR_Hubbard/  ham_T, Ham_chem, Ham_U, ham_T2, Ham_U2, &
              &     ham_Tperp, ham_h0, Mz, Continuous
         ...
         ham_h0     = 0.0d0   ! Pinning field strength. Local magnetic 
                              ! field on site i=0, orbital=1
         Mz         = .true.  ! When true, sets the M_z-Hubbard model: 
                              ! Nf=2, demands that N_sun is even, HS field 
                              ! couples to the z-component of magnetization; 
                              ! otherwise, HS field couples to the density
         Continuous = .false. ! Uses (T: continuous; F: discrete) HS transformation
#ifdef MPI
         If (Irank_g == 0 ) then
#endif
#ifdef TEMPERING
            ...
            REWIND(unit_para)
            READ(unit_para, NML=VAR_Hubbard)
            close(unit_para)
         ...
         CALL MPI_BCAST(ham_h0    ,  1,MPI_REAL8    ,0,Group_Comm,ierr)
...
CALL h5lexists_f(group_id, "var_hubbard", link_exists, ierr)
                       if ( .not. link_exists ) then
                         call h5gcreate_f(group_id, "var_hubbard", group_id2, ierr)
                         call h5gclose_f(group_id2, ierr)
                       endif
         call test_attribute(group_id, "var_hubbard", "ham_t", ham_T, ierr)
         call test_attribute(group_id, "var_hubbard", "ham_chem", Ham_chem, ierr)
         call test_attribute(group_id, "var_hubbard", "ham_u", Ham_U, ierr)
         call test_attribute(group_id, "var_hubbard", "ham_t2", ham_T2, ierr)
         call test_attribute(group_id, "var_hubbard", "ham_u2", Ham_U2, ierr)
         call test_attribute(group_id, "var_hubbard", "ham_tperp", ham_Tperp, ierr)
         call test_attribute(group_id, "var_hubbard", "ham_h0", ham_h0, ierr)
         call test_attribute(group_id, "var_hubbard", "mz", Mz, ierr)
         call test_attribute(group_id, "var_hubbard", "continuous", Continuous, ierr)
