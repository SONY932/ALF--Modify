...
**测试配置**：2x2 lattice, Beta=1.0, Dtau=0.02, Ltrot=50

| Ham_h | K_G   | <G_r>    | GaussViol | Acceptance |
|-------|-------|----------|-----------|------------|
| 0.02  | 3.91  | ~0.001   | ~1.999    | 98.4%      |
| 0.1   | 3.11  | ~-0.001  | ~2.003    | 98.5%      |
| 0.5   | 2.30  | ~0.001   | ~1.997    | 98.0%      |
| 2.0   | 1.61  | ~0.002   | ~1.997    | 98.6%      |

**观察**：
- K_G 从 1.6 变化到 3.9，但 <G_r> 和 GaussViol 没有显著变化
- 这符合预期：边界更新只占 2/50 = 4%，且时间关联近似有 O(Δτ) 误差

**结论**：
- 约束机制正确实现（编译OK、Green精度OK、acceptance正常）
- GaussViol ≈ 2 是测量方式（时间关联近似）的固有限制
- 要观察更明显的 K_G 效应，需要：更小的 Δτ 或更长的热化
